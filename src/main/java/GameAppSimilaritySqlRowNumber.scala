...
object GameAppSimilaritySqlRowNumber {
  def main(args: Array[String]): Unit = {
    ...
    // 转成rdd进行处理
    val rdd: RDD[Row] = userPkgInfoDF.rdd
    ...
    val similarityRank: DataFrame = session.sql("select pkg1name,pkg2name,value,row_number() OVER (DISTRIBUTE BY pkg1name SORT BY value DESC) rn from app_run_similarity")
